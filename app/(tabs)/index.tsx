...
function EntryCard({
  ...
  return (
    <View style={[styles.card, { borderColor: border, backgroundColor: card }]}>
      ...
      {!!item.placeName && (
        <Text style={[styles.meta, { color: muted }]}>{item.placeName}</Text>
      )}
      <Text style={[styles.meta, { color: muted }]}>
      ...
      <View
        style={[
          ...
          {
            backgroundColor:
              item.syncStatus === "pending" ? "#f59e0b22" : "#16a34a22",
      ...
      <View style={styles.actionsRow}>
        <TouchableOpacity
          ...
          style={[
            styles.mapButton,
            { borderColor: tint, backgroundColor: card },
          ]}
        >
          <Text
            style={{ color: tint, fontWeight: "600", fontFamily: Fonts.sans }}
          >
            Map
          </Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity
          ...
          style={[
            styles.mapButton,
            { borderColor: "#dc2626", backgroundColor: card },
          ]}
        >
          <Text
            style={{
  ...
  const loadEntries = useCallback(
    async (isRefresh = false) => {
      ...
      try {
        ...
        const [syncedEntries, queuedEntries] = await Promise.all([
          getEntries(token),
          getQueuedEntries(),
        ]);
        ...
        result.syncedCount > 0
          ? `${result.syncedCount} entry${result.syncedCount > 1 ? "ies" : "y"} synced${
              result.remainingCount > 0
                ? `, ${result.remainingCount} still pending`
                : ""
            }.`
          : result.remainingCount > 0
    ...
    <View style={[styles.container, { backgroundColor: background }]}>
      <View style={styles.headerRow}>
        <View>
          ...
          <Text style={[styles.lastSyncText, { color: muted }]}>
            {lastSyncLabel}
          </Text>
        </View>
        <View style={styles.headerActions}>
          ...
          <TouchableOpacity
            ...
            style={[styles.logoutButton, { backgroundColor: tint }]}
          >
            <Text style={[styles.logoutText, { color: background }]}>
              Logout
            </Text>
          </TouchableOpacity>
        </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <Text style={[styles.empty, { color: muted }]}>
            No entries yet. Capture your first one!
          </Text>
        }
